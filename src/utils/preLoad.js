...
function preLoad(routes, pathname, query, store) {
  ...
  branch.forEach(({ route, params, pathname }) => {
    ...
    promises.push(_preLoad({ route: route, store, match, query }));
  });
...
const generateInitialPropsKey = (pathName, query) => `__preload-${pathName}-${query.toString()}-props__`;

const resolveGetInitialStateFunction = async ({ route }) => {
  const getInitialStateArray = [];
  // for Router
  ...
  if (route.Component) {
    ...
    if (typeof WrapperComponent.load === "function") {
      const loadAbleComponent = await WrapperComponent.load();
      ...
      const preLoadComponent = WrapperComponent;
  ...
  if (getInitialStateArray.length) {
    return async ({ store, pathName, params, query }) => {
      ...
      return res.filter(Boolean).reduce((s, c) => {
        if (!c) {
          return s;
        }
        s.cookies = { ...s.cookies, ...c.cookies };
...
const _preLoad = async ({ route, store, match, query }) => {
...
function preLoadWrapper(preLoad) {
...
function AutoInjectInitialProps(Component) {
